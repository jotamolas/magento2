...
class ocaepack extends AbstractCarrier implements
    CarrierInterface
{
    protected $_code = 'ocaepack';
    ...
    public function collectRates(RateRequest $request)
    {
        ...
        if ($request->getAllItems()) {
            ...
            if ($response['state'] == 'ok') {
                $shippingPrice = $response['Precio'] * 1.21; //seteo IVA
                if ($this->getConfigData('oca_recargo')) { //seteo recargo
                    $shippingPrice = $shippingPrice * (1 + ($this->getConfigData('oca_recargo') / 100));
                }
                $result = $this->_rateResultFactory->create();
                $method = $this->_rateMethodFactory->create();
                $method->setCarrier('ocaepack');
                $method->setCarrierTitle(
                    $this->getConfigData('title')
                );
                $method->setMethod('ocaepack');
